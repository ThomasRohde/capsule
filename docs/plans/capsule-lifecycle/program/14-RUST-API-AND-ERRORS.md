...
Review-only projection is deliberately separate from execution authority:

```rust
pub trait LifecyclePreview {
    fn preview_copy(&self, request: CopyPreviewRequest)
        -> Result<CopyPlanReport, WorkspaceError>;
}
```

`CopyPlanReport` is bounded, serializable review data. It contains no path,
source/destination capability, SQLite connection, prepared plan or confirmation
nonce, and cannot be passed to an executor. It uses
`org.sqlite-capsule.copy-preview/1`. The authoritative `prepare_copy` below
consumes opaque host capabilities and retains a non-serializable prepared plan.
