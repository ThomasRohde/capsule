...
1. Implement `prepare_copy` as a dry-run returning identity effects, dataset
   actions, estimated row counts, sensitivity prompts, dependencies, expected
   application digest and output constraints.
2. Implement exact duplicate using SQLite's consistent backup API or the
   repository-approved equivalent. Preserve capsule and revision identity.
...
6. Implement selective fork only when all affected dataset policies and
   dependencies permit it. Sensitive datasets default to omitted.
