# Acceptance gate — M06: Apply selected changes to a new target-derived copy
...
- [x] There is no in-place merge code path.
- [x] All inputs remain byte-identical.
- [x] The target application compartment is unchanged.
- [x] Unresolved conflicts and failed validation block publication.
- [x] The published output corresponds exactly to the reviewed plan.
