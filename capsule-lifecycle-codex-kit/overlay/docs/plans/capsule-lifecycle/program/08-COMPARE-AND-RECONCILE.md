...
- exact source, target and optional base evidence;
- application/data compatibility;
- selected changes with canonical old/new value hashes;
- explicit conflict resolutions;
- expected row effects;
- target-derived output revision identity;
- validation list;
- plan digest.

Avoid storing unbounded values in audit logs. The executable plan may contain
selected values under strict size bounds; durable audit contains counts and
digests.
