...
| --- | --- | --- | --- | --- | --- | --- |
| v0.2 unsigned | Existing policy | Yes | Not signature-preserving; authoring conversion only | Metadata/schema only | No generic v0.2 reconcile | Unavailable until M08 accepts a versioned signed legacy-adapter contract |
| v0.2 signed | Existing policy | Yes | No | Metadata/schema only | No | Unavailable until M08 accepts a versioned signed legacy-adapter contract |
| v0.3 unsigned | Existing explicit trust policy | Yes | No; signed contract authority is required | Read-only structural comparison only | No transforming reconcile | No application upgrade |
